    ...
    <xsl:choose>
        <xsl:when test="count(br_) > 0">
            <xsl:call-template name="verse">
                ...
                <xsl:with-param name="verse-type" select="br_[1]/preceding-sibling::*[name() = 'wers_wciety' or name() = 'wers_akap' or name() = 'wers_cd' or name() = 'wers_do_prawej'][1]" />
            </xsl:call-template>
            <xsl:for-each select="br_">
              <!-- Each BR_ tag "consumes" text after it -->
                ...
                <xsl:call-template name="verse">
                    ...
                    <xsl:with-param name="verse-type" select="following-sibling::*[count(preceding-sibling::br_) = $lnum+1 and (name() = 'wers_wciety' or name() = 'wers_akap' or name() = 'wers_cd' or name() = 'wers_do_prawej')][1]" />
                </xsl:call-template>
            </xsl:for-each>
        </xsl:when>
        <xsl:otherwise>
